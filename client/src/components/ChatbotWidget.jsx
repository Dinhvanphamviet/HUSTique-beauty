  ...
  return (
    <div className="fixed bottom-6 right-6 z-50">
        {isChatOpen ? (
                ...
                <ScrollArea className="flex-1 p-4 bg-pink-50">
                    {messages.map((msg, index) => (
                        <div key={index} className={`mb-4 flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                            <div className={`inline-block p-3 rounded-xl max-w-[80%] shadow-md break-words ${
                                msg.role === 'user' 
                ...
                <div className="p-3 border-t border-gray-200 bg-white flex items-center space-x-2">
                    ...
                    <button
                        ...
                        className={`p-3 rounded-2xl transition-all flex items-center justify-center ${
                            isLoading ? 'bg-gray-400 cursor-not-allowed' : 'bg-pink-500 text-white hover:bg-pink-600'
                        }`}>
            ...
            <button
                onClick={() => openChat()}
                className="bg-pink-500 text-white p-4 rounded-full shadow-xl hover:shadow-2xl transform hover:scale-110 transition-all duration-300 flex items-center justify-center"
